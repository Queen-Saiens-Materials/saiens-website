  ...
  return (
    <main className="flex flex-1 flex-col bg-(--black) text-(--white)">
      <section className="relative isolate overflow-hidden">
        {/*
          The original site uses a single tall source photo (four glowing
          stone/gem graphics fading into a sunset bridge scene) as one
          continuous background spanning the hero through the RSVP form —
          not a separate baked-text hero image. Replicate that by anchoring
          one background image behind all of these sections together.
        */}
        ...
        <div className="relative flex min-h-[70vh] w-full flex-col items-center justify-center gap-4 px-6 py-24 text-center">
        ...
        <div className="relative mx-auto flex w-full max-w-3xl flex-col gap-6 px-6 py-16 text-center">
          <p className="font-semibold text-(--white)/90">
            在《好感之夜》中，我們以「讓建築・真正開始循環」為主軸，從材料革新、空間應用到永續工藝，重新串連人、建築與環境的關係。透過循環思維與跨界創新，讓每一份資源被更長久地使用，也讓建築不只承載生活，更成為邁向未來的永續行動。
          </p>
        </div>

        <div className="relative mx-auto grid w-full max-w-3xl grid-cols-1 gap-x-10 gap-y-14 px-6 py-20 sm:grid-cols-2">
        ...
        <div className="relative mx-auto flex w-full max-w-3xl flex-col gap-3 px-6 py-8 text-center">
          <h3 className="font-(family-name:--font-jost) text-2xl tracking-tight md:text-3xl">
            拆解未來生活的想像，從建築、結構到物件，展開一場以材料為核心的三維對話。
          </h3>
        </div>

        <div className="relative mx-auto flex w-full max-w-3xl flex-col gap-2 px-6 py-8 text-center">
        ...
        <div className="relative mx-auto flex w-full max-w-xl flex-col gap-8 px-6 py-16">
